...
(function($, _, Marionette){
  ...
  var injectComponent = function(component, attributes, transclusion){
    ...
    return  "<cp-"+component+" "+id+" "+attributes+">".replace(escaper, escapeChar)
          + "';\n " +  "Marionette.Renderer.componentLoader(this, '"+component+"', '"+id+"', '"+transclusion+"');" + "\n__p+='"
          + "</cp-"+component+">".replace(escaper, escapeChar);
  }

  Marionette.Renderer.componentLoader = function(view, tagName, uniqueAttr, transclusion, scope) {
    if (scope == null) {
      scope = window;
    }
